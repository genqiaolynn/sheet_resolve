...
def get_direction(points_list, box_polygon, raw_line, image):
    ...
    cv2.imwrite(os.path.join(r'E:\December\math_12_18\1_18\test_img\save2\\', 'THRESH_BINARY_INV' + '.jpg'), binary)
...
def fld_demo1(image, sheet_dict, split_x):
    ...
    for line in line_block:
        ...
        for index, ele in enumerate(block_list):
            ...
            # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\\' + str(index) + '_extend_lines' + '.jpg', image)
            # print('ok')
            ...
            # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\\' + str(index) + '_longitudinal' + '.jpg', image)
            # print(line)
...
def filter_line_in_mark(lines, sheet_dict):
    ...
    for line in lines_tmp:
        ...
        cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\mark.jpg', image)
    return lines_tmp
...
def fld_demo3(image, sheet_dict, split_x):
    ...
    # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\raw_lines.jpg', image)
    ...
    for index, region_box in enumerate(sheet_dict):
        ...
        # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\enlarge_box.jpg', image)
...
def fld_demo4(image, sheet_dict, split_x):
            ...
            for line2 in longitudinal_list_tmp:
                ...
                if True in decision1 and True in decision2:
                    ...
                    if abs(line_start_lan - bbox_xmin) < abs(bbox_xmax - line_end_lan):      # 延长左边
                        ...
                        extend_line_lan = LineString([(line1[0] - i, line1[1]), (line1[2], line1[1])])  # 横向
                        ...
                        for j in range(int(bbox_xmax - line_end_lan)):
                            ...
                            if pixel == (255, 255, 255):
                                print(j)
                                break
                        extend_line_lan = LineString([(line1[0], line1[1]), (line_end_lan + j, line1[1])])# 横向
                    ...
                    if abs(line_start_lon - bbox_ymin) < abs(bbox_ymax - line_end_lon):   # 向上延长
                        ...
                        extend_line_lon = LineString([(line2[0], line_start_lon - m), (line2[0], line2[3])])  # 纵向
                        ...
                        extend_line_lon = LineString([(line2[0], line2[1]), (line2[0], line_end_lon + n)])  # 纵向
                    landscape_list.append(extend_line_lan)
                    longitudinal_list.append(extend_line_lon)
                    # image1 = image.copy()
                    # image2 = image.copy()
                    ...
                    #          cv2.LINE_AA)
                    # # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\\' + str(index) + '_raw_lines' + '.jpg', image1)
                    # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\\' + '_raw_lines' + '.jpg', image1)
                    #
                    # # extend line   draw
                    ...
                    #          cv2.LINE_AA)
                    #
                    # # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\\' + str(index) + '_extend_lines' + '.jpg', image2)
                    # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save2\\' + '_extend_lines' + '.jpg', image2)
    ...
    for index, box_polygon in enumerate(roi_box_polygon):
        for line1 in landscape_list:
            for line2 in longitudinal_list:
                # 这部分对原先出来的线进行增大
                extend_line_lan = LineString([(line1.bounds[0], line1.bounds[1]), (line1.bounds[2], line1.bounds[3])])   # 横向增加5个像素

                extend_line_lon = LineString([(line2.bounds[0], line2.bounds[1]), (line2.bounds[2], line2.bounds[3])])          # 纵向的增加5个像素

                decision1 = [extend_line_lan.within(box_polygon), extend_line_lan.contains(box_polygon)]
                decision2 = [extend_line_lon.within(box_polygon), extend_line_lon.contains(box_polygon)]
                if True in decision1 and True in decision2:
                    bbox_xmin, bbox_ymin, bbox_xmax, bbox_ymax = box_polygon.bounds[0], box_polygon.bounds[1], \
                                                                 box_polygon.bounds[2], box_polygon.bounds[3]
                    ...
                    direction = get_direction([(xp, yp)], box_polygon, (extend_line_lan, extend_line_lon), image)
                    if direction != None:
                        points_dict = {}
    ...
    tree.write(r'E:\December\math_12_18\1_18\test_img\save2\3.xml')
    e = time.time()
    cost_time = e - t
