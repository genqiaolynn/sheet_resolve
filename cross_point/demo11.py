...
def get_std_row(image):
    ...
    # 计算一个一维卷积
    # sum_x_axis = img_mtx.sum(axis=0) / (height*255)
    # kernel = np.array([-1, 0, 1])
    # sobel_filter = scipy.signal.convolve(sum_x_axis, kernel)
    #
...
def judge_cross_point_and_direction(points_list, image, name, thresh=9, pixel_thresh=450):
    ...
    for ele0 in points_list:
        ...
        point_direction_dict['point'] = ele0
        ...
        cv2.imwrite(r'E:\111_4_26_test_img\aa\save2\box.jpg', image)
...
def get_point_and_direction(image, sheet_dict, split_x, name, save_path):
    t = time.time()
    # img_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    # ret, binary = cv2.threshold(img_gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    fld = cv2.ximgproc.createFastLineDetector()
    dlines = fld.detect(image)
    canvas = fld.drawSegments(image, dlines)
    cv2.imwrite(os.path.join(save_path, 'canvas.jpg'), canvas)
    ...
    for dline in new_lines:
        ...
        # cv2.putText(image, text, (x1, y1), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 0), 1)
    ...
    for line1 in landscape_list_tmp:
        for line2 in longitudinal_list_tmp:
            ...
            # 这里需要考虑延长左边还是右边
            # 说明线更接近矩形框的起点,这种情况延长左边到框
            # if abs(line_start_lan - bbox_xmin) <= abs(bbox_xmax - line_end_lan):
            #     extend_line_lan = LineString([(bbox_xmin, line1[1]), (line1[2], line1[3])])
            # else:
            #     extend_line_lan = LineString([(line1[0], line1[1]), (bbox_xmax, line1[3])])
            #
            # # 说明线更接近矩形框的起点,这种情况延长左边到框
            # if abs(line_start_lon - bbox_ymin) <= abs(bbox_ymax - line_end_lon):
            #     extend_line_lon = LineString([(line2[0], bbox_ymin), (line2[2], line2[3])])
            # else:
            #     extend_line_lon = LineString([(line2[0], line2[1]), (line2[2], bbox_ymax)])
    ...
    # tree.write(os.path.join(save_path, name + '_raw_points' + '.xml'))
    # cv2.imwrite(os.path.join(save_path, name + '_raw_points' + '.jpg'), image)
    ...
    # differ_new1 = np.array(sort_by_x)[:, 0][1:] - np.array(sort_by_x)[:, 0][:-1]
    ...
    #
    # point_direction_list = judge_cross_point_and_direction(points_list_new_y, image, name, thresh=15, pixel_thresh=700)
    # print(point_direction_list)
    #
    # template = r'F:\exam\sheet_resolve\exam_info\000000-template.xml'
    # tree = ET.parse(template)
    # for points_ in point_direction_list:
    #     points = points_['point'][0]
    #     create_xml(str(points_['std']) + '_' + str(points_['mean']) + '_' + points_['direction'], tree,
    #                int(points[0]), int(points[1]), int(points[0] + 1), int(points[1] + 1))
    # tree.write(os.path.join(save_path, name + '.xml'))
...
if __name__ == '__main__':
    ...
    img_path = r'E:\111_4_26_test_img\aa\a'
    rm_dir(img_path)
    ...
    for ele in img_list:
        if ele.endswith('.jpg'):
            ...
            # cv2.imwrite(os.path.join(save_path, ele), image_)
            name = ele.split('.')[0]
            get_point_and_direction(image, sheet_dict, split_x, name, save_path)