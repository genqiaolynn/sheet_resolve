...
def fld_demo4(image, sheet_dict, name, save_path):
    ...
    if len(points_new) == 1:
        ...
        index_x = list(np.where(differ_x >= 6.0)[0])     # 符合条件的说明不是同一个点,这些点都要加进列表里去
        ...
        index_y = list(np.where(differ_y >= 6.0)[0])     # 符合条件的说明不是同一个点,这些点都要加进列表里去
            ...
            if len(index_x) > 2:
                ...
                for index_a, ele in enumerate(index_x[1:]):
                    ...
                    if len(block_one) == 1:
                        ...
                        points_new_list.append([(x_mean, y_mean), (block_one[max_index_lan[-1]][1][0], block_one[max_index_lon[-1]][1][1]), block_one[-1][-1]])
            ...
            if len(index_y) > 2:
                for index_a, ele in enumerate(index_y[1:]):
                    ...
                    if len(block_one) == 1:
                        ...
                        block_one[max_index_lan[-1]][1][0], block_one[max_index_lon[-1]][1][1]), block_one[-1][-1]])
                print('points_new_list:', points_new_list)
        else:
