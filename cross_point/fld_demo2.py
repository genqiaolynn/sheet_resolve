...
def get_direction(points_list, box_polygon, raw_line, image):
    image_height, image_width = image.shape[:2]
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ret, binary = cv2.threshold(gray, 240, 255, cv2.THRESH_BINARY_INV)  # raw
    cv2.imwrite(os.path.join(r'E:\December\math_12_18\1_18\test_img\save\\', 'THRESH_BINARY_INV' + '.jpg'), binary)
    ...
    for points in points_list:
        ...
        raw_line_lan = LineString([(raw_line_lan_.bounds[0], raw_line_lan_.bounds[1]),
                                   (raw_line_lan_.bounds[2], raw_line_lan_.bounds[3])])

        raw_line_lon = LineString([(raw_line_lon_.bounds[0], raw_line_lon_.bounds[1]),
                                   (raw_line_lon_.bounds[2], raw_line_lon_.bounds[3])])

        # 水平向右
        # box_polygon = Polygon([(xmin, ymin), (xmax, ymin), (xmax, ymax), (xmin, ymax)])

        right_area = Polygon([(point_x, point_y - 7), (right_boarder, point_y - 7),
                              (right_boarder, point_y + 7), (point_x, point_y + 7)])

        decision = [raw_line_lan.within(right_area), raw_line_lan.contains(right_area)]
        if True in decision:
            DIR_RIGHT = True

        # 水平向左
        left_area = Polygon([(left_boarder, point_y - 7), (point_x, point_y - 7),
                             (point_x, point_y + 7), (left_boarder, point_y + 7)])
        decision = [raw_line_lan.within(left_area), raw_line_lan.contains(left_area)]
        if True in decision:
            DIR_LEFT = True

        # box_polygon = Polygon([(xmin, ymin), (xmax, ymin), (xmax, ymax), (xmin, ymax)])
        # 垂直向上
        top_area = Polygon([(point_x - 7, top_boarder), (point_x + 7, top_boarder),
                            (point_x + 7, point_y), (point_x - 7, point_y)])
        decision = [raw_line_lon.within(top_area), raw_line_lon.contains(top_area)]
        if True in decision:
            DIR_UP = True

        # 垂直向下
        bottom_area = Polygon([(point_x - 7, point_y), (point_x + 7, point_y),
                            (point_x + 7, bottom_boarder), (point_x - 7, bottom_boarder)])
        decision = [raw_line_lon.within(bottom_area), raw_line_lon.contains(bottom_area)]
        if True in decision:
            DIR_DOWN = True
...
def fld_demo1(image, sheet_dict, split_x):
    ...
    for line in line_block:
        ...
        for index, ele in enumerate(block_list):
            ...
            # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + str(index) + '_extend_lines' + '.jpg', image)
            # print('ok')
...
def filter_line_in_mark(lines, sheet_dict):
    ...
    for line in lines_tmp:
        ...
        cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\mark.jpg', image)
    return lines_tmp
...
def fld_demo3(image, sheet_dict, split_x):
    ...
    # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\raw_lines.jpg', image)
    ...
    for index, region_box in enumerate(sheet_dict):
        ...
        # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\enlarge_box.jpg', image)
...
def fld_demo4(image, sheet_dict, split_x):
    ...
    for index, box_polygon in enumerate(roi_box_polygon):
        for line1 in landscape_list:
            for line2 in longitudinal_list:
                # 这部分对原先出来的线进行增大
                ...
                if True in decision1 and True in decision2:
                    ...
                    # 说明线的左边离框更近,保证左边到框
                    if abs(line_start_lan - bbox_xmin) < abs(bbox_xmax - line_end_lan):      # 延长左边
                        i = 0
                        for i in range(int(line_start_lan - bbox_xmin)):
                            pixel = img.getpixel((int(line_start_lan) - i, int(line1[1])))
                            if pixel == (255, 255, 255):
                                print(i)
                                break
                            # print(pixel)
                            # if image[int(line_start_lan) - i, int(line1[1])] == np.array([255, 255, 255]):
                            #     print('ok')
                            # index_new = np.where(image[line_start_lan[i, line1[1]]] == np.array([255, 255, 255]))
                            # print(index_new)

                        # extend_line_lan = LineString([(box_polygon.bounds[0], line1[1]), (line1[2], line1[3])])  # 横向   raw

                        extend_line_lan = LineString([(line1[0] - i, line1[1]), (line1[2], line1[3])])  # 横向

                    else:   # 延长右边
                        j = 0
                        for j in range(int(bbox_xmax - line_end_lan)):
                            pixel = img.getpixel((int(line_end_lan) + j, int(line1[1])))
                            if pixel == (255, 255, 255):
                                print(j)
                                break
                        extend_line_lan = LineString([(line1[0], line1[1]), (int(line_end_lan) + int(j), line1[3])])# 横向

                    # 说明下面离框更近,保证下面的点
                    if abs(line_start_lon - bbox_ymin) < abs(bbox_ymax - line_end_lon):   # 向上延长
                        m = 0
                        for m in range(int(line_start_lon - bbox_ymin)):
                            pixel = img.getpixel((int(line_start_lon) - m, int(line2[1])))
                            if pixel == (255, 255, 255):
                                print(m)
                                break

                        extend_line_lon = LineString([(line2[0], line2[1] - m), (line2[0], line2[3])])  # 纵向

                    else:    # 向下延长
                        n = 0
                        for n in range(int(bbox_ymax - line_end_lon)):
                            pixel = img.getpixel((int(line_end_lon), int(line2[1]) + n))
                            if pixel == (255, 255, 255):
                                print(n)
                                break

                        extend_line_lon = LineString([(line2[0], line2[1]), (line2[0], line2[3] + n)])  # 纵向

                    # all_line.append((extend_line_lan, extend_line_lon))
                    # image1 = image.copy()
                    # image2 = image.copy()
                    # cv2.line(image1, (int(line1[0]), int(line1[1])), (int(line1[2]), int(line1[3])), (0, 255, 0), 1,
                    #          cv2.LINE_AA)
                    # cv2.line(image1, (int(line2[0]), int(line2[1])), (int(line2[2]), int(line2[3])), (255, 255, 0), 1,
                    #          cv2.LINE_AA)
                    # # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + str(index) + '_raw_lines' + '.jpg', image1)
                    # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + '_raw_lines' + '.jpg', image1)
                    #
                    # # extend line   draw
                    # cv2.line(image2, (int(extend_line_lan.bounds[0]), int(extend_line_lan.bounds[1])),
                    #          (int(extend_line_lan.bounds[2]), int(extend_line_lan.bounds[3])), (0, 255, 0), 1,
                    #          cv2.LINE_AA)
                    #
                    # cv2.line(image2, (int(extend_line_lon.bounds[0]), int(extend_line_lon.bounds[1])),
                    #          (int(extend_line_lon.bounds[2]), int(extend_line_lon.bounds[3])), (0, 255, 0), 1,
                    #          cv2.LINE_AA)
                    #
                    # # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + str(index) + '_extend_lines' + '.jpg', image2)
                    # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + '_extend_lines' + '.jpg', image2)
    ...
    #     lan = ele[0].bounds
    #     lon = ele[1].bounds
    #     if len(lan) == 0 or len(lon) == 0:
    #         break
    #     create_xml('points', tree, int(lan[0]), int(lan[1]), int(lan[2]), int(lan[3]))
    #     create_xml('points', tree, int(lon[0]), int(lon[1]), int(lon[2]), int(lon[3]))
    # tree.write(r'E:\December\math_12_18\1_18\test_img\save\3.xml')

    for ele in all_line:
        print(ele)
        lan = ele[0].bounds
        lon = ele[1].bounds
        cv2.line(image, (int(lan[0]), int(lan[1])), (int(lan[2]), int(lan[3])), (0, 255, 0), 1, cv2.LINE_AA)
        cv2.line(image, (int(lon[0]), int(lon[1])), (int(lon[2]), int(lon[3])), (0, 255, 0), 1, cv2.LINE_AA)
    cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\extend_new_img.jpg', image)
    ...
    #
    #
    #                 # extend_line_lon = LineString([(line2[0], box_polygon.bounds[1]), (line2[0], box_polygon.bounds[3])]) # 纵向
    #
    ...
    #                 xp, yp, xp1, yp1, xp2, yp2 = '', '', '', '', '', ''
    #                 if cond3:
    #                     (xp1, yp1) = raw_line_lan.intersection(raw_line_lon).bounds[:2]
    #                     # intersect_point_list.append((xp, yp))
    #                 elif cond1:
    #                     (xp2, yp2) = extend_line_lan.intersection(extend_line_lon).bounds[:2]
    #                     print((xp, yp))
    #                 if cond3 and cond1:
    #                     xp, yp = xp1, yp1
    #                 elif not cond3 and cond1:
    #                     xp, yp = xp2, yp2
    #                     # points_list.append((xp, yp))
    #                 # 对点进行判断,重合的点就不再进行以下的操作,不重合就进行下面的操作
    #                 # print(xp)
    #                 # print(yp)
    #                 # x_axis = [int(ele[0]) for ele in points_list]
    #                 # y_axis = [int(ele[1]) for ele in points_list]
    #                 # try:
    #                 #     index_x = np.where(np.abs(np.array(x_axis) - xp) < 5.0)[0]
    #                 #     index_y = np.where(np.abs(np.array(y_axis) - yp) < 5.0)[0]
    ...
    #                 image1 = image.copy()
    #                 image2 = image.copy()
    #                 cv2.line(image1, (int(line1[0]), int(line1[1])), (int(line1[2]), int(line1[3])), (0, 255, 0), 1, cv2.LINE_AA)
    #                 cv2.line(image1, (int(line2[0]), int(line2[1])), (int(line2[2]), int(line2[3])), (255, 255, 0), 1, cv2.LINE_AA)
    #                 cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + str(index) + '_raw_lines' + '.jpg', image1)
    #                 # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + '_raw_lines' + '.jpg', image1)
    #
    #                 # extend line   draw
    #                 cv2.line(image2, (int(extend_line_lan.bounds[0]), int(extend_line_lan.bounds[1])),
    #                          (int(extend_line_lan.bounds[2]), int(extend_line_lan.bounds[3])), (0, 255, 0), 1, cv2.LINE_AA)
    #
    #                 cv2.line(image2, (int(extend_line_lon.bounds[0]), int(extend_line_lon.bounds[1])),
    #                          (int(extend_line_lon.bounds[2]), int(extend_line_lon.bounds[3])), (0, 255, 0), 1, cv2.LINE_AA)
    #
    #                 cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + str(index) + '_extend_lines' + '.jpg', image2)
    #                 # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save\\' + '_extend_lines' + '.jpg', image2)
    #                 print(cond1)
    #
    #
    #                 direction = get_direction([(xp, yp)], box_polygon, (raw_line_lan, raw_line_lon), image)
    #                 if direction != None:
    #                     points_dict = {}
    #                     points_dict['points'] = (xp, yp)
    #                     points_dict['direction'] = direction
    #                     points_dict['extend_line'] = (extend_line_lan, extend_line_lon)
    #                     points_list_all.append(points_dict)
    # print(points_list_all)
    # template = r'F:\exam\sheet_resolve\exam_info\000000-template.xml'
    # tree = ET.parse(template)
    # for ele in points_list_all:
    #     points = ele['points']
    #     extend_line = ele['extend_line']
    #     create_xml('points', tree, int(points[0]), int(points[1]), int(points[0] + 1), int(points[1] + 1))
    #     # lan
    #     create_xml('lan_line', tree, int(extend_line[0].bounds[0]), int(extend_line[0].bounds[1]),
    #                int(extend_line[0].bounds[2]), int(extend_line[0].bounds[3]))
    #     # lon
    #     create_xml('lon_line', tree, int(extend_line[1].bounds[0]), int(extend_line[1].bounds[1]),
    #                int(extend_line[1].bounds[2]), int(extend_line[1].bounds[3]))
    # tree.write(r'E:\December\math_12_18\1_18\test_img\save\3.xml')
    # e = time.time()
    # cost_time = e - t
    ...
    #     extend_line_lon = LineString()
    #     for line in lines_tmp:
    #         if int(line[2]) - int(line[0]) < 10:  # 纵向
    #             raw_line_lan = LineString([(line[0], line[1]), (line[2], line[3])])
    #             decision = [raw_line_lan.within(box_polygon), raw_line_lan.contains(box_polygon),
    #                         raw_line_lan.overlaps(box_polygon)]
    #             if True in decision:
    #                 extend_line_lan = LineString([(box_polygon.bounds[0], line[1]), (box_polygon.bounds[2], line[3])])
    #
    #         elif int(line[3]) - int(line[1]) < 10:  # 横向
    #             raw_line_lon = LineString([(line[0], line[1]), (line[2], line[3])])
    #             decision = [raw_line_lon.within(box_polygon), raw_line_lon.contains(box_polygon),
    #                         raw_line_lon.overlaps(box_polygon)]
    #             if True in decision:
    #                 extend_line_lon = LineString([(line[0], box_polygon.bounds[1]), (line[2], box_polygon.bounds[3])])
    #
    #             cond1 = extend_line_lan.intersects(extend_line_lon)  # T, L, 十交叉
    #             cond2 = extend_line_lan.crosses(extend_line_lon)  # 十字交叉
    #             print(cond1)
    ...
    #     longitudinal_length = []
    #
    #     for line in landscape_list:  # 横向
    #         raw_line_lan = LineString([(line[0], line[1]), (line[2], line[3])])
    #         decision = [raw_line_lan.within(box_polygon), raw_line_lan.contains(box_polygon), raw_line_lan.overlaps(box_polygon)]
    #         if True in decision:
    #             extend_line_lan = LineString([(box_polygon.bounds[0], line[1]), (box_polygon.bounds[2], line[3])])
    #
    #     for line in longitudinal_list:  # 纵向
    #         raw_line_lon = LineString([(line[0], line[1]), (line[2], line[3])])
    #         decision = [raw_line_lon.within(box_polygon), raw_line_lon.contains(box_polygon), raw_line_lon.overlaps(box_polygon)]
    #         if True in decision:
    #             extend_line_lon = LineString([(line[0], box_polygon.bounds[1]), (line[2], box_polygon.bounds[3])])
    #
    #         cond1 = extend_line_lan.intersects(extend_line_lon)  # T, L, 十交叉
