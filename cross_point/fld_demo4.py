...
def get_direction(points_and_lines, box_polygon, image):
    ...
    if abs(raw_line_lan_.bounds[3] - point_y) > abs(point_y - raw_line_lan_.bounds[1]):
...
def fld_demo4(image, sheet_dict, split_x):
    ...
    for index, box_polygon in enumerate(roi_box_polygon):
        ...
        for points_ in points_list:
            ...
            if np.mean(np.array(pixel_point)) <= 200:
        ...
        points_all_list.append(points_new_list)
        # 合并points_new_list成一个list
        # points_new_list_tmp = [ele for ele in points_new_list]

    # image1 = image.copy()
    # for ele1 in points_all_list:
    #     for ele in ele1:
    #         line1 = ele[1][0].bounds
    #         line2 = ele[1][1].bounds
    #         cv2.line(image1, (int(line1[0]), int(line1[1])), (int(line1[2]), int(line1[3])), (0, 255, 0), 2,
    #                  cv2.LINE_AA)
    #         cv2.line(image1, (int(line2[0]), int(line2[1])), (int(line2[2]), int(line2[3])), (255, 255, 0), 2,
    #                  cv2.LINE_AA)
    #     # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save3\\' + str(index) + '_raw_lines' + '.jpg', image1)
    # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save3\\' + '_raw_lines' + '.jpg', image1)
            ...
            #          cv2.LINE_AA)
            #
            # # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save3\\' + str(index) + '_extend_lines' + '.jpg', image2)
            # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save3\\' + '_extend_lines' + '.jpg', image2)



        # 这里得到的点可能不准确,需要判断每个点是不是真实的,筛选掉假点

        # s_box_l_up = [bbox_xmin, bbox_ymin, bbox_xmin + 100, bbox_ymin + 100]    # 左上
        # s_box_r_up = [bbox_xmax - 100, bbox_ymin, bbox_xmax, bbox_ymin + 100]    # 右上
        # s_box_l_bottom = [bbox_xmin, bbox_ymax, bbox_xmin + 40, bbox_ymax - 40]  # 左下
        # s_box_r_bottom = [bbox_xmax - 40, bbox_ymax - 40, bbox_xmax, bbox_ymax]  # 右下
        # cv2.rectangle(image, (int(s_box_l_up[0]), int(s_box_l_up[1])), (int(s_box_l_up[2]), int(s_box_l_up[3])), (0, 0, 255), 1)
        # cv2.imwrite(r'E:\December\math_12_18\1_18\test_img\save3\enlarge_box_left.jpg', image)
    ...
    # for points in points_all_list:
    #     points = points['points']
    #     create_xml(str(points), tree, int(points[0]), int(points[1]), int(points[0] + 1), int(points[1] + 1))
